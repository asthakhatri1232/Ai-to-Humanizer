...
export default function StatsPanel({ changeCount, humanScore, wordCount }) {
  ...
  return (
    <div style={{
      ...
      <h3 style={{ color: '#94a3b8', fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '1rem' }}>
        Analysis Results
      </h3>
      <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
        <StatCard
          label="Words Changed"
          value={changeCount > 0 ? changeCount : '—'}
          color="#8b5cf6"
          icon="✏️"
        />
        <StatCard
          label="Human Score"
          value={humanScore > 0 ? `${humanScore}%` : '—'}
          color="#10b981"
          icon="🎯"
        />
        <StatCard
          label="AI Detection Risk"
          value={humanScore > 0 ? risk.label : '—'}
          color={humanScore > 0 ? risk.color : '#94a3b8'}
          icon="🛡️"
        />
        <StatCard
